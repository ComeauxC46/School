...
/*
	...
	Revised by Walter Daugherity September 2015
...
Token Token_stream::get() // read characters from cin and compose a Token
{
    ...
    case '0': case '1': case '2': case '3': case '4': //forgot case '0'****
...
{
    for (int i = 0; i<var_table.size(); ++i) // <= to <, loop up to the size range error****
...
{
    for (int i = 0; i<var_table.size(); ++i) //range error** changes <= to <*****
...
double term()
{
    ...
    while(true) {
        ...
        case '*':
		{
            left *= primary();
            t = ts.get();
			break; // no break statement so program was falling through when an * was entered *****
		}	
        case '/':
            {    
                double d = primary();
